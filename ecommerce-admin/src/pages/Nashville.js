...
const Nashville = () => {
  ...
  for (let i = 0; i < productState.length; i++) {
    if(productState[i].stockNashville !== 0 && productState[i].stockNashville !== "0" && productState[i].stockNashville !== null){
      data1.push({
        key: i + 1,
        ...
        amount: productState[i].stockNashville + " (" + productState[i].toNashville + ")" ,
        action: (
          <>
            <Link
  ...
  return (
    <div>
      <h3 className="mb-4 title">Nashville</h3>
      <div>
