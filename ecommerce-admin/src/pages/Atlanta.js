...
const Atlanta = () => {
  ...
  for (let i = 0; i < productState.length; i++) {
    if(productState[i].stockAtlanta !== 0 && productState[i].stockAtlanta !== "0" && productState[i].stockAtlanta !== null){
      data1.push({
        key: i + 1,
        ...
        amount: productState[i].stockAtlanta  + " (" + productState[i].toAtlanta + ")",
        action: (
          <>
            <Link
  ...
  return (
    <div>
      <h3 className="mb-4 title">Atlanta</h3>
      <div>
