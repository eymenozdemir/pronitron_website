...
import { createRefurbished, resetState, getARefurbished, updateARefurbished } from "../features/refurbished/refurbishedSlice";
...
const Addrefurbished = () => {
  ...
  const refurbishedState = useSelector((state) => state?.refurbished);
  const categoryState = useSelector((state) => state?.pCategory?.pCategories);
  const { isSuccess, isError, isLoading, createdProduct, updatedProduct, productName } = refurbishedState;
  ...
  useEffect(() => {
    ...
    if (getProdId !== undefined) {
      dispatch(getARefurbished(getProdId));
    } else {
      dispatch(resetState());
    }
  }, [getProdId]);
  ...
  const onDrop = (acceptedFiles) => {
    ...
    dispatch(uploadDownloadables(validFiles));
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const data = {
        ...
        refImg: imgState,
        refDownloadables: downloadableState,
      };

      if (getProdId !== undefined) {
        dispatch(updateARefurbished({ id: getProdId, ...data }));
      } else {
        dispatch(createRefurbished(data));
      }
      setTimeout(() => {
        dispatch(resetState());
        navigate("/admin/refurbisheds");
